...
public class AllSatExample {
    ...
    private static final ImmutableSet<SolverContextFactory.Solvers> SOLVERS_WITH_INTEGERS =
            ImmutableSet.of(MATHSAT5, SMTINTERPOL, Z3, PRINCESS, CVC4, YICES2);
    private static final ImmutableSet<SolverContextFactory.Solvers> SOLVERS_WITH_BITVECTORS =
    ...
    private List<List<BooleanFormula>> allSatBooleans1()
        ...
        return prover.allSat(
                new BasicProverEnvironment.AllSatCallback<>() {
     ...
     */
    private List<List<Model.ValueAssignment>> allSatBooleans2()
            throws InterruptedException, SolverException {
        bfmgr = context.getFormulaManager().getBooleanFormulaManager();
     ...
     */
    private List<List<Model.ValueAssignment>> allSatIntegers()
            throws InterruptedException, SolverException {
        bfmgr = context.getFormulaManager().getBooleanFormulaManager();
        ifmgr = context.getFormulaManager().getIntegerFormulaManager();

        // formula ((1 <= a <= 10) && (1 <= b <= 10) && (a >= 2 * b)) with 25 models
        NumeralFormula.IntegerFormula a = ifmgr.makeVariable("a");
        NumeralFormula.IntegerFormula b = ifmgr.makeVariable("b");
     ...
     * For integer formulas, we can implement the allsat-loop and collect all models when iterating.
     */
    private List<List<Model.ValueAssignment>> allSatIntegers2()
            throws InterruptedException, SolverException {
        bfmgr = context.getFormulaManager().getBooleanFormulaManager();
        ifmgr = context.getFormulaManager().getIntegerFormulaManager();
     ...
     */
    private List<List<Model.ValueAssignment>> allSatBitvectors()
            throws InterruptedException, SolverException {
        bfmgr = context.getFormulaManager().getBooleanFormulaManager();
